...
- 时间复杂度：最好时间复杂度为$O(1)$，最坏时间复杂度为$O(n^2)$，平均时间复杂度计算时需要引入**有序度**和**无序度**。
  ...
  - **无序度**：数组中具备无序关系的元素对个数。`3,2,1`的无序度为`3`，无序对为`3,2、3,1、2,1`。
...
- 元素移动的次数等于数组的逆序度。
- 当对一个逆序度为K的数组进行排序时，冒泡排序需要K次交换，每次交换时需要3次赋值语句(非Python)，交换总耗时$3*K$，插入排序需要K次数据移动，总耗时$K$。因此插入排序性能更好。

### 选择排序

选择排序也分已经排序和未排序区，每次从未排序区中选取最大或最小的元素，插入已排序区。

![选择排序](Raw/基础算法/选择排序.png)
...
- 时间复杂度：最好时间复杂度为$O(n^2)$，最坏时间复杂度为$O(n^2)$，平均时间复杂度为$O(n^2)$。
- 空间复杂度：插入排序是原地排序，其空间复杂度为$O(1)$。
- 稳定性：选择排序是从未排序元素中选取最小的元素，然后和前面的元素交换位置，因此会破坏稳定性。
