...
```python
def dynamic_planning(weights, limit_weight):
    """
    采用动态规划解决0-1背包问题
    :param weights:商品的重量信息
    :param limit_weight:背包能承受的最大重量
    :return:max_weight, queue
    """
    numbers = len(weights)
    ...
    for i in range(1, numbers):
        ...
        # 将第i个商品放入被背包
        for j in range(limit_weight - weights[i] + 1):
            if status[i - 1][j] is True:
    ...
    for i in range(numbers - 1, 0, -1):
        # (i,j)的状态只能由(i-1,j)或(i-1,j-weights[i])
        if (j - weights[i] >= 0) and status[i - 1][j - weights[i]] is True:
            queue.append(i)
            j -= weights[i]
    if j > 0:
        queue.append(0)
...
def test_dynamic_planning():
    ...
    weights = [7, 1, 4]
    assert dynamic_planning(weights, 5) == (5, [1, 2])

    weights = [2, 2, 4, 6, 5, 5, 3]
    assert dynamic_planning(weights, 9) == (9, [3, 6])
...
```python
def optimize_dynamic_planning(goods, limit_weight):
    """
    采用动态规划解决0-1背包问题,空间复杂度为O(limit_weight+1)
    :param goods:商品的重量信息
    :param limit_weight:背包能承受的最大重量
    :return:
    """
    numbers = len(goods)
    status = [False for _ in range(limit_weight + 1)]
    ...
    if goods[0] <= limit_weight:
        status[goods[0]] = True
        queues[goods[0]].append(0)

    # 整理状态
    for i in range(1, numbers):
        # 将第i个商品放入被背包
        # 顺序会出现重复计算
        for j in range(limit_weight - goods[i], -1, -1):
            if status[j] is True:
                status[j + goods[i]] = True
                queues[j + goods[i]] = queues[j] + [i]
    ...
    for i in range(limit_weight, -1, -1):
        if status[i] is True:
            ...
            best_index = i
            break
    best_queue = queues[best_index]
    return max_weight, best_queue


def test_optimize_dynamic_planning():
    weights = [5]
    assert optimize_dynamic_planning(weights, 6) == (5, [0])

    weights = [1, 2, 4]
    assert optimize_dynamic_planning(weights, 6) == (6, [1, 2])

    weights = [2, 2, 4, 6, 3]
    assert optimize_dynamic_planning(weights, 9) == (9, [3, 4])

    weights = [7, 8, 9]
    assert optimize_dynamic_planning(weights, 5) == (0, [])

    weights = [7, 1, 4]
    assert optimize_dynamic_planning(weights, 5) == (5, [1, 2])

    weights = [2, 2, 4, 6, 5, 5, 3]
    assert optimize_dynamic_planning(weights, 9) == (9, [3, 6])
```
