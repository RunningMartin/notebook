...
- 清位`^&`：`0b1100^&0b1010=0b0100`，以第一个操作数为标准，相同的置为`0`；不同时，以第一个操作数为准。`0b1010^&0b1100=0b0010`
- 左移位`<<`：`0b1100<<3=0b1100000`
- 右移位`>>`：`0b1100>>3=0b1`
...
- 两个操作数，如果有一个操作数的类型确定，则结果和该类型确定操作数类型相同的类型确定值。另一个将被推断或隐式转换为该类型确定操作数的类型。
- 如果两个操作数都是类型不确定值，则结果也是类型不确定值，运算过程中，两个操作数的类型为默认类型(优先级：`complex128`>`float64`>`rune`>`int`)。

### 移位运算符规则

- 左操作数为类型确定值(必定为整数)，则结果也是一个和左操作数类型相同的类型确定值。
- 如果左操作数是一个类型不确定值并且右操作数是常量，则左操作数被视作为整数(`rune`>`int`)。结果也是一个类型不确定值且默认类型和左操作数相同。
- 如果左操作数是一个类型不确定值并且右操作数是非常量，则左操作数先被转换为运算结果期待类型，如果没有指定，则将左操作数默认类型视作为它的期待设想类型，如果该期待设想类型不是整数类型，则编译报错。结果是一个类型为此期待设想类型的类型确定值。
...
func main() {
	//类型不确定值，默认类型int rune complex128
	const x, y, z = 2, 'A', 3i
	// 类型确定值
	var a, b int = x, y
	fmt.Println("预期类型为int32", reflect.TypeOf(x+y))
	fmt.Println("预期类型为int", reflect.TypeOf(a+b))
	fmt.Println("预期类型为int", reflect.TypeOf(a+x))
	fmt.Println("预期类型为complex", reflect.TypeOf(z+y))
	fmt.Println("预期类型为int", reflect.TypeOf(a<<1))
	fmt.Println("预期类型为int", reflect.TypeOf(x<<1))
	// 类型不确定
	var i = int32(64)
	fmt.Println("左操作数是一个类型不确定值且右操作数是非常量，预期类型为int32", reflect.TypeOf(i>>a))

	var j int64 = 1 << i
	fmt.Println("左操作数是一个类型不确定值且右操作数是非常量，预期类型为int64", reflect.TypeOf(j))
	//var _=1.0<<i //默认类型不是整数类型，则报错

	var l = uint(32)
	// 1的默认类型被推断为int，32bit OS则结果为0,64bit OS则结果为2^32
	var n  = 1 << l
	fmt.Println("左操作数是一个类型不确定值且右操作数是非常量，预期类型为int", reflect.TypeOf(n))
	// 采用int64修饰后，1的类型推断为int64
	var m  int64= 1 << l
	fmt.Println("左操作数是一个类型不确定值且右操作数是非常量，预期类型为int64", reflect.TypeOf(m))
}
```
